...
const Summary = ({name, weather, temperature, congestion}: SummaryProps) => {
  return (
    <div className={cn("flex justify-between p-6 items-center bg-gray-100")}>
      <div>
        ...
        <div className={cn("flex items-end")}>
          ...
          <p className={cn("opacity-50 text-xl font-bold")}>{temperature}°C</p>
        </div>
      </div>
      <div className={cn("text-sm font-bold rounded-md bg-gray-400 text-white px-3 py-1")}>{congestion}</div>
    </div>
  );
};
